...
qnode counts. `pseudo_legal_moves` emits ascending by `(from, to)`; any replacement must
match. Use `check_kind_over_ordered`, which distinguishes `SET MISMATCH` from `ORDER
MISMATCH`. A best-move-and-score identity test structurally **cannot** catch this.
